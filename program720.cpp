...
# define MAXOPENEDFILES 20      // Maximum number of files that we can open 
# define MAXINODE 5             // Maimum files that we can create
...
void DisplayHelp()
{
    ...
    printf("man : it is used to display the specific manual command\n");
            ...
            )
{
    ...
    {
        printf("Description : This command is used to delete file from file system\n");

        printf("Usage : delete\n");     
    }
    else if(strcmp(name,"crea") == 0)
    {
        printf("Description : This command is used to terminate the Marvellous CVFS\n");
            ...
            )
{
    ...
    printf("Current inodes remaining : %d", superobj.FreeInodes);
    // Filters
    ...
    for(i = 0; i < MAXINODE; i++)
    {
        if(uareaobj.UFDT[i] != NULL)
        {
            if(strcmp(uareaobj.UFDT[i]->ptrinode->FileName, name) == 0)
            {
                ...
                uareaobj.UFDT[i]->FileSize = 0;
                uareaobj.UFDT[i]->ActualFileSize = 0;
                uareaobj.UFDT[i]->LinkCount = 0;
                uareaobj.UFDT[i]->Permission = 0;
                uareaobj.UFDT[i]->FileType = 0;
                uareaobj.UFDT[i]->ReferenceCount = 0;
                uareaobj.UFDT[i]->Buffer = NULL;
                uareaobj.UFDT[i]->next = NULL;
    ...
    while(1)
    {
        ...
        else if(iCount == 2)
        {
            ...
            else if(strcmp(Command[0], "unlink") == 0)
            {
                ...
                {
                    printf("File Unlink operation successfully performed\n");
                }
                else if(iRet == ERR_FILE_NOT_EXIST)
                {
                    printf("Unable to do unlink activity");
                }
                //else if(iRet == )
            }
            else
